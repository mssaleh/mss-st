...
definition(
    name: "MQTT Bifrost",
    namespace: "mssaleh",
    author: "Mohammed Saleh, John Johnson and Jeremiah Wuenschel",
...
def actionColor(device, attribute, value) {
    switch (attribute) {
        ...
        break
        case "color":
    		def rrr = Integer.parseInt(value.substring(1,3),16)
    		def ggg = Integer.parseInt(value.substring(3,5),16)
    		def bbb = Integer.parseInt(value.substring(5,7),16)
			def r = rrr / 255
    		def g = ggg / 255
    		def b = bbb / 255
    		def h = 0
			...
			def hue_val = Math.round(h * 100)
            def sat_val = Math.round(s * 100)
    		def colormap = [:]
    		colormap = ["hue": hue_val as int, "saturation": sat_val as int]
			log.debug "hue: ${hue_val}, sat: ${sat_val}, colormap: ${colormap}"
